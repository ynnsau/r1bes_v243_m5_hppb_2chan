...
(
    ...
    // for new ncp method
    output prefetch_rw_v2_state_t prefetch_rw_curr_state, // for AFU specifically 
...
logic direct_ncp_r;
logic bvalid_r, hb_rvalid_r;
(* preserve_for_debug *) prefetch_rw_v2_state_t  state, next_state, prev_state;

assign prefetch_page_data = db_rdata_reg;
...
(* preserve_for_debug *) logic [15:0] bvalid_stall_cnt_n, bvalid_stall_cnt_r;
(* preserve_for_debug *) logic [15:0] rvalid_stall_cnt_n, rvalid_stall_cnt_r;
(* preserve_for_debug *) logic bvalid_stall_debug;
(* preserve_for_debug *) logic rvalid_debug, rvalid_stall_debug;

assign rvalid_debug = (rvalid && 
                        (state == NCP_CHECK || state == NCP_WRITE || 
                         state == NCP_ABORT || state == NCP_WRITE_RESP || 
                         state == NCP_WRITE_DATA));
...
function void device_bias_read();
    arid = 12'h2; // TODO: check during weekly sync
    aruser = 6'b110000; // target HDM, Device-bias NCR
    arvalid = 1'b1;
    araddr = prefetch_page_addr_r;
endfunction

function void host_bias_read();
    arid = 12'h3; // TODO: check during weekly sync
...
always_ff @(posedge axi4_mm_clk) begin
    if (!axi4_mm_rst_n) begin
        ...
        abort_cnt_reg <= '0;
        success_cnt_reg <= '0;
        bvalid_stall_cnt_r <= '0;
        rvalid_stall_cnt_r <= '0;
        direct_ncp_r <= '0;
        bvalid_r <= '0;
        hb_rvalid_r <= '0;
        prev_state <= IDLE;
        ...
        state <= next_state;
        bvalid_stall_cnt_r <= bvalid_stall_cnt_n;
        rvalid_stall_cnt_r <= rvalid_stall_cnt_n;
        ncp_timeout_cnt_r <= ncp_timeout_cnt_n;
        ...
        unique case(state)
            ...
            NCP_WRITE_RESP, WRITE_RESP_TIMEOUT: begin
                if (bvalid) begin
                    bvalid_r <= '1;
                end
                if (rvalid) begin
...
always_comb begin
    ...
    bvalid_stall_cnt_n = bvalid_stall_cnt_r;
    rvalid_stall_cnt_n = rvalid_stall_cnt_r;
    ncp_timeout_cnt_n = ncp_timeout_cnt_r;
    bvalid_stall_debug = 1'b0;
    rvalid_stall_debug = 1'b0;
    addr_issued = 1'b0;
    unique case(state)
        IDLE: begin
            bvalid_stall_cnt_n = '0;
            rvalid_stall_cnt_n = '0;
            if (start_prefetch) begin
                addr_issued = 1'b1;
                next_state = DB_READ_ADDR;
            end else if (direct_ncp) begin
                addr_issued = 1'b1;
        ...
        DB_READ_ADDR: begin
            if(abort_op =='0) begin
                if (arvalid & arready) begin
                    next_state = DB_READ_DATA;
                end
            end else begin
                next_state = ABORT_HB_VALID_CHECK;
            end
        end
        DB_READ_DATA: begin
            if (abort_op == '0) begin
        ...
        HB_READ_ADDR: begin
            if (abort_op == '1 || addr_seen == '1) begin // edge case, addr seen triggered by CPU shows up here
                next_state = ABORT_HB_VALID_CHECK;
        ...
        HB_READ_DATA: begin
            if (abort_op == '1) begin
                ...
                next_state = NCP_ABORT;
            end else if (addr_seen == '1) begin // addr seen lower priority (just to be safe)
                if (rvalid_stall_cnt_r < 16'd16) begin // addr seen too early to be true
                    // XXX, 16 cycle is just a guess
                    next_state = ABORT_HB_VALID_CHECK;
                end else begin
                    next_state = NCP_WRITE;
                end
            end
            if (rvalid_stall_cnt_r > 16'd1024) begin // XXX, turns out this can be > 512 cycles ...
                rvalid_stall_debug = 1'b1;
            end else begin
                rvalid_stall_cnt_n = rvalid_stall_cnt_r + 16'd1;
            end
        end
        NCP_WRITE: begin
        ...
        NCP_WRITE_DATA: begin
            if (wready & wvalid) begin
                next_state = NCP_WRITE_RESP;
            end else begin
                next_state = NCP_WRITE_DATA;
            end
        end
        NCP_WRITE_RESP: begin
            if (bvalid & bready) begin
                new_success_cnt = success_cnt_reg + 64'd1;
                if (direct_ncp_r) begin
                    next_state = IDLE;
                end else begin
                    next_state = HB_VALID_CHECK;
                end
            end
            if (bvalid_stall_cnt_r > 16'd512) begin
                if (direct_ncp_r) begin
                    next_state = IDLE;
                end else begin
                    next_state = WRITE_RESP_TIMEOUT;
                    ncp_timeout_cnt_n = ncp_timeout_cnt_r + 32'd1;
                end
                bvalid_stall_debug = 1'b1;
            end else begin
                bvalid_stall_cnt_n = bvalid_stall_cnt_r + 16'd1;
            end
        end
        WRITE_RESP_TIMEOUT: begin
            if (bvalid_r & hb_rvalid_r) begin
                new_abt_cnt = abort_cnt_reg + 64'd1;
                next_state = IDLE;
            end
        end
        ABORT_HB_VALID_CHECK: begin
            if (rvalid & rready) begin
                next_state = NCP_ABORT;
            end
        end
        NCP_ABORT: begin
            new_abt_cnt = abort_cnt_reg + 64'd1;
            next_state = WAIT_LOW_RVALID;
        end
        HB_VALID_CHECK: begin
            if (rvalid & rready) begin
                next_state = WAIT_LOW_RVALID;
            end
        end
        WAIT_LOW_RVALID: begin
